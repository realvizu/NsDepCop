...
namespace Codartis.NsDepCop.Core.Test.Util
{
    public class FileHelperTests : FileBasedTestsBase
    {
        ...
        {
            Action a = () => FileHelper.GetFolderPaths(GetFilePathInTestClassFolder("whatever"), 0).ToList();
        ...
        {
            Action a = () => FileHelper.GetFolderPaths(GetFilePathInTestClassFolder(@"NoSuchFolder"), 3).ToList();
        ...
        {
            var folderPaths = FileHelper.GetFolderPaths(GetFilePathInTestClassFolder(@"Level3\Level2\Level1"), 3).ToList();
        ...
        {
            var folderPaths = FileHelper.GetFilenameWithFolderPaths("a.txt", GetFilePathInTestClassFolder(@"Level3\Level2\Level1"), 3).ToList();
            folderPaths.Should().HaveCount(3);
        ...
        {
            var filePaths = FileHelper.FindInParentFolders("test.txt", GetFilePathInTestClassFolder(@"Level3\Level2\Level1"), 1).ToList();
        ...
        {
            var filePaths = FileHelper.FindInParentFolders("test.txt", GetFilePathInTestClassFolder(@"Level3\Level2\Level1"), 3).ToList();
